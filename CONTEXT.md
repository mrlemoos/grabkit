...
When the HTTP status is not OK, **`error`** is a **`GrabkitError`** with `statusCode`, parsed `body`, derived `message`, and **`errors`** (`JsonApiError[]` from JSON:API error documents). Still returned, not thrown.
